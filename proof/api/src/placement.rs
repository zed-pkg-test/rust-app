use crate::runtime_control::{RuntimeControlSigner, RuntimeControlTarget};
...
impl PlacementService {
    ...
    fn runtime_request_payload<T: Serialize + ?Sized>(
        ...
        if key.execution_class.backend() == ExecutionBackend::Firecracker {
            ...
            serde_json::to_value(body).map_err(|err| HostAttemptError::Unavailable(err.to_string()))
        }
    }
...
mod tests {
    ...
    fn firecracker_payload_requires_runtime_control_secret() {
        let service =
            PlacementService::new(vec!["http://faas".into()], vec!["http://fc".into()]).unwrap();
        let key = ShardKey {
    ...
    fn standard_faas_payload_remains_unsigned_for_compatibility() {
        let service = PlacementService::new(vec!["http://faas".into()], vec![]).unwrap();
        let key = ShardKey {
