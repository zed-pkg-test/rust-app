...
for ((i = 0; i < ${#RW_PATHS[@]}; i++)); do
  ...
  [[ "$rw_path" != / ]] || fatal 'refusing host root read-write'
...
SBPL
  ...
  cat >>"$PROFILE" <<'SBPL'
; Explicit local-development IP networking. Seatbelt treats localhost as a
; distinct endpoint class on current macOS, so permit it explicitly in addition
; to the general IP rule. Unix-domain sockets remain denied by default.
