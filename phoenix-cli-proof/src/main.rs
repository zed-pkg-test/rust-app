...
enum Commands {
    ...
    /// Deploy a compiler-produced Phoenix release to a Firecracker runtime cell.
    PhoenixDeploy {
        #[arg(default_value = "dist-phoenix")]
        artifact_dir: PathBuf,
        #[arg(long)]
        tenant_id: String,
    ...
    /// Execute provider-neutral performance/cost benchmark drivers and validate fresh receipts.
    Benchmark {
        /// Benchmark project root. All plan/receipt/output paths are confined below this directory.
        #[arg(long, default_value = ".")]
        root: PathBuf,
        /// ores.provider-benchmark.plan/v1 JSON plan, relative to --root.
        #[arg(long)]
        plan: PathBuf,
        /// Optional aggregate receipt output path, relative to --root. Must not already exist.
        #[arg(long)]
        out: Option<PathBuf>,
        /// Validate the plan and command declarations without executing benchmark drivers.
        #[arg(long)]
        check: bool,
        /// Emit machine-readable aggregate/check JSON.
        #[arg(long)]
        json: bool,
    },
