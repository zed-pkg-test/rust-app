...
use critical_sections::{
    ...
    CriticalSectionDeployments, DeployCriticalSectionRequest, EXECUTION_CLASS,
    ISOLATION_CLASS, PROFILE as CRITICAL_SECTION_PROFILE,
    TENANCY_CLASS as CRITICAL_SECTION_TENANCY,
};
use placement::{
    ExecutionBackend, ExecutionClass, PlacementError, PlacementService, RuntimePlacement,
};
use runtime_control::RuntimeControlSigner;
use reqwest::Client;
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/v1/critical-sections/deployments", post(deploy_critical_section))
        .route(
...
async fn critical_section_operation(
    ...
    let deployment = state
        ...
        .get(deployment_id)
        .await
        .ok_or_else(|| api_error(StatusCode::NOT_FOUND, "critical-section deployment not found"))?;
    ...
    let signed = state
        ...
        .sign(
            ...
            &deployment.tenant_id,
            &runtime.shard_id,
            execution_class_name(ExecutionClass::DurableActor),
            execution_backend_name(ExecutionBackend::Firecracker),
