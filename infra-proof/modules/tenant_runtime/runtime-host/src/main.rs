...
fn validate_critical_section_request(req: &CriticalSectionRequest) -> Result<(), ApiError> {
    ...
    let lease_ok = req
        .lease_ms
        .is_some_and(|lease| (1..=300_000).contains(&lease));
    let request_id_ok = req.request_id.as_deref().is_some_and(|request_id| {
        !request_id.is_empty() && request_id.len() <= 256 && !request_id.contains('\0')
    });
    let token_ok = req.token.as_ref().is_some_and(|token| {
        token.runtime_epoch > 0
            && token.owner_epoch > 0
            && token.sequence > 0
            && token.sequence <= MAX_CRITICAL_SECTION_SEQUENCE
    });
    let valid = match req.operation {
        CriticalSectionOperation::Acquire => lease_ok && request_id_ok && req.token.is_none(),
        CriticalSectionOperation::Renew => lease_ok && req.request_id.is_none() && token_ok,
        CriticalSectionOperation::Release => {
            req.lease_ms.is_none() && req.request_id.is_none() && token_ok
        }
    };
    if valid {
        Ok(())
    ...
    T: Serialize + RuntimeRequestIdentity,
{
    ...
    consume_runtime_nonce(&mut consumed, &contract.nonce, expires_at, now)
        .map_err(|message| api_error(StatusCode::UNAUTHORIZED, message))
}
...
mod tests {
    ...
    fn validates_critical_section_operation_shapes() {
        ...
        let envelope = critical_section_envelope(&acquire);
        assert_eq!(envelope["execution_class"], "durable_actor");
        assert_eq!(envelope["execution_backend"], "firecracker");
        assert_eq!(envelope["tenant_id"], "tenant-1");
        assert_eq!(envelope["runtime_epoch"], 7);

        let mut missing_request_id = acquire.clone();
        missing_request_id.request_id = None;
        assert!(validate_critical_section_request(&missing_request_id).is_err());

        let mut invalid_release = acquire.clone();
        invalid_release.operation = CriticalSectionOperation::Release;
        invalid_release.lease_ms = None;
        invalid_release.token = Some(CriticalSectionToken {
            runtime_epoch: 7,
            owner_epoch: 3,
            sequence: 11,
        });
        assert!(
            validate_critical_section_request(&invalid_release).is_err(),
            "release must reject acquire request identity"
        );

        let mut release = invalid_release;
        release.request_id = None;
        assert!(validate_critical_section_request(&release).is_ok());
    }
